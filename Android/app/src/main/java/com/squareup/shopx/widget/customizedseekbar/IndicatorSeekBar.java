package com.squareup.shopx.widget.customizedseekbar;
...
public class IndicatorSeekBar extends View {
    ...
    private String mIndicatorTextFormat = "${PROGRESS}mi";
    //tick marks
    ...
    private void measureTickTextsBonds() {
        if (needDrawText()) {
            ...
            mTickTextsHeight = mRect.height() + SizeUtils.dp2px(mContext, 8);//with the gap(3dp) between tickTexts and track.
        }
    }
    ...
    private void refreshSeekBarLocation() {
        ...
        if (needDrawText()) {
            mTextPaint.getTextBounds("j", 0, 1, mRect);
            mTickTextY = mPaddingTop + mCustomDrawableMaxHeight + Math.round(mRect.height() - mTextPaint.descent()) + SizeUtils.dp2px(mContext, 8);
    ...
    private void drawTickMarks(Canvas canvas) {
        ...
        for (int i = 0; i < mTickMarksX.length; i++) {
            ...
            if (mTickMarksDrawable != null) {
                ...
                if (i <= thumbPosFloat) {
                    Bitmap bitmap = getDrawBitmap(getResources().getDrawable(R.drawable.isb_marker_black_icon), false);
                    canvas.drawBitmap(bitmap, mTickMarksX[i] - mUnselectTickMarksBitmap.getWidth() / 2.0f, mProgressTrack.top - mUnselectTickMarksBitmap.getHeight() / 2.0f, mStockPaint);
                } else {
                    Bitmap bitmap = getDrawBitmap(getResources().getDrawable(R.drawable.isb_marker_icon), false);
                    canvas.drawBitmap(bitmap, mTickMarksX[i] - mUnselectTickMarksBitmap.getWidth() / 2.0f, mProgressTrack.top - mUnselectTickMarksBitmap.getHeight() / 2.0f, mStockPaint);
                }
                continue;
            }
    ...
    private String getProgressString(float progress) {
        if (mIsFloatProgress) {
            return FormatUtils.fastFormat(progress, mScale) + "mi";
        } else {
            return String.valueOf(Math.round(progress)) + "mi";
        }
    }
    ...
    public void setIndicatorTextFormat(String format) {
        this.mIndicatorTextFormat = "${PROGRESS} %";
        initTextsArray();
        updateStayIndicator();
    }
